...
describe('Email Authentication', function () {
	this.timeout(500);

	before(function (done) {
		setup.init.initUser(done);
	});

	it('should authenticate valid email, password combination', function (done) {
		var loginCredentials = {
			client : "android",
			c_tz_offset : 330,
			email : setup.data.user.email,
			password : setup.data.user.password
		};
		request(setup.url)
	...
	it('should reject invalid email', function (done) {
		...
		.end(function (err, res) {
			res.body.status.should.equal('error');
			res.body.errcode.should.equal(2001);
			done();
		});
	});
